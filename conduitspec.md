...
decides the wire form. **Also out of scope for Lane A:** some store calls are not time-bounded, as
on main today. For Code Mode: `start`'s `create()`, the drive's journal, pause and
settle writes, and the reads of `resume`'s read-side guard for a code row. For BOTH kinds: the
`claimForResume` await, which runs before the row's kind is known, so a stalled claim hangs a direct
resume too. A store that commits, or accepts a read, and never answers hangs that call. Bounding these needs
attempt-fenced late-completion recovery and is a follow-up. A timer on the code-row guard was tried and removed: a
code row has no drive, so the timer was a second writer beside the guard, and a second writer needs a latch the
code path does not otherwise have. What Lane A bounds: the kind lookup that follows the claim, for both kinds (a
stalled lookup answers `unknown` within the drive budget; it has one writer); after it, every store call
on the direct path — the direct-row resume guard, the drive, and every settle and fallback write — and the
terminalizing writes of a code row's resume guard. **Threat-model notes:** host dependencies — a
custom invoker, store, or log sink — are trusted, so hostile values thrown or returned by them are outside the
threat model. The direct arm still builds its settle plan — the classification, the formatted cause, and one
snapshot of the result — before it takes its settle latch, so a value it cannot classify or serialize fails before
the latch rather than after it; that is a design rule, not a proof of totality against a hostile host. An
...
sentinel, `direct_call`, `program`, the provenance fields, `request_keys`), and the
R1 spec's §3.1 writer-floor rule makes such changes cheap only before a first release. **Then R3a —
preview packaging:** a versioned, packed artifact with alpha install notes and ONE governance property
demonstrable in the first-run path (approve → the exact approved call runs once; deny → nothing ran) — not a
public npm release; full R3 (lifecycle, install/upgrade/recovery guidance, docs) is unchanged. **Alpha
window: three weeks from the day the R3a artifact is first distributed to someone outside the repo, with a
default outcome** — if no adopter needs per-client profiles by then, public R3 ships Code Mode only and
Lanes B/C wait for a named pull; if an adopter needs profiles, Lanes B and C land before public R3.
**Reversal trigger:** a qualified adopter committing to Code Mode without profiles but requiring
npm install → the R3a packaging and its governance demonstration are still built (they are the release), but
the three-week wait is skipped: public R3 follows R3a directly. Not in the minimum:
trace viewer, GraphQL, cloud, desktop, SSO. Basis: a competitive read of executor.sh (same code-mode gateway
thesis, published and priced, code-mode only, per-tool allow/approve/block policy, no published
approval-binding, replay, egress, or invariant story) and a codex cross-model review (`gpt-5.6-sol`,
medium) that confirmed the Lane-A-first constraint, rejected "packaging only" as a description of R3, and
noted that Lanes B/C set compatibility expectations once published even without schema changes. The bounded
window with a default outcome is the agent's addition: without it both the alpha and the profiles decision
wait on an adopter who does not yet exist.
