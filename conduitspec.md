...
window: three weeks, with a default outcome** — if no adopter needs per-client profiles by then, public
R3 ships Code Mode only and Lanes B/C wait for a named pull; if an adopter needs profiles, Lanes B and C land
before public R3. **Reversal trigger:** a qualified adopter committing to Code Mode without
profiles but requiring npm install → skip the alpha, public R3 directly after Lane A. Not in the minimum:
