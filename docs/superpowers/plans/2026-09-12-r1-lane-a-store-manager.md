...
**The guard phase itself runs under the timer (Greptile on PR #61, second P1):** `onExpire` is assigned AT drive creation, before the first guard read, to a guard-phase handler — never left undefined until the run attaches. The guard reads run inside `Promise.race([guardPhase(), expiry])` where `expiry` is a deferred the handler resolves; on expiry the handler takes the latch, writes `settleDirectBounded(executionId, resumeAttemptId, { status: "failed", error: ConduitExecutionInterrupted })`, calls `finishEarly()`, and `resume` returns that terminal (or `unknown` if the write did not land). When the guard passes and the run attaches, Task 10's arm reassigns `onExpire` to `expireDirect(run)`; because the latch is shared, whichever handler fires first wins and the other is a no-op. Pinned in Task 10's tests: a direct resume whose `tools.get` never returns, with `lifecycle` installed → `resume` resolves `failed`/`ConduitExecutionInterrupted` within the drive budget plus the settle budget, `retention` resolves, `finished` resolves, no upstream call.

**Every early exit completes the lifecycle (Greptile on PR #61, first P1):** once `lifecycle` has been published, a direct resume that terminalizes in the guard — corrupt pause, `ConduitCatalogChanged`, `ConduitScopeRevoked`, TTL expiry, guard-read timeout, or the prep-window catch — must resolve the same two promises, or Lane B holds the admission slot forever. `terminalize` and the expiry branch therefore end with `directDrive.finishEarly()` for direct rows, where:
...
8. **Greptile on PR #61 (2026-09-13, two rounds):** round 1 — lifecycle promises pending on guard exits (P1, fixed: `finishEarly()`), three `Source` literals without `generation: 0`, a duplicated conflict mapper in the `start` snippet, one machine-specific path (all fixed). Round 2 — the guard-phase timer had no handler until the run attached (P1, fixed: guard phase raced against expiry with the handler assigned at creation); "plan should be HTML" — **false positive by repo convention**: `docs/superpowers/plans/*.md` holds nine prior plans, CLAUDE.md's artifact rule reserves HTML for published artifacts and keeps Markdown for git-tracked prose where diff reviewability wins, and the writing-plans skill writes Markdown by design.
