...
export async function runStdioServer(opts: { env?: NodeJS.ProcessEnv } = {}): Promise<void> {
  ...
  // Old bin.ts wrapped this sequence in a local try/catch that prints the
  // bare error and exits BEFORE main().catch() can add its
  // "[ConduitMcp] Fatal:" prefix — reproduce that exactly so serve-path
  // startup failures are byte-identical (Task 4: shared with bin.ts's
  // --doctor call site via openStoreFromEnv).
  let env: ResolvedEnv;
  let store: ConduitStore;
  try {
    ({ env, store } = await openStoreFromEnv(opts.env));
  } catch (error) {
