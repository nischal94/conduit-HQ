...
/**
 * The canonical, symlink-resolved absolute path of `target` when it does
 * not fully exist yet — for the fresh-install case where the default
 * directory has no inode to compare against.
 *
 ...
 * exactly as the string bug in the main check does — so `..` is resolved
 ...
 * exist cannot be a symlink and cannot alias anything. Two not-yet-
 * existent paths are "the same default" only when this canonical form is
 * identical, which no symlink can forge.
 */
function canonicalOfMissing(target: string): string {
 ...
 *
 *  - Both the default and `stateDir` resolve to existing directories →
 ...
 *  - The default does NOT exist yet (fresh install) → it has no inode; a
 *    `stateDir` "is the default" only when it is ALSO genuinely absent
 *    (no directory entry at all, checked with `lstat` so a DANGLING
 *    symlink — which could be retargeted — does not qualify) and its
 *    `canonicalOfMissing` form equals the default's. With both genuinely
 *    absent, nothing in either path can forge the match, and the
 *    legitimate fresh-install auto-start (default absent, `stateDir` ==
 *    default constant) still classifies as default.
 *  - Exactly one resolves to a directory → they cannot be the same
 *    directory (an alternate spelling of an existing directory
 *    necessarily exists) → not default. This also covers a dangling
 ...
 * in one place with no bypass. Exported for the ARC-pass-3 regression
 * tests; not part of the client's public surface.
 */
